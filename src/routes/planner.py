...
@planner_bp.route("/plans/<user_id>", methods=["GET"])
def get_user_plans(user_id):
    """사용자의 스터디 플랜 조회 (문자열 ID 지원)"""
    try:
        # 날짜 범위 파라미터 처리
...
def create_plan():
    ...
    try:
        ...
        # 새 플랜 생성
        plan = StudyPlan(
            user_id=data["user_id"],  # 문자열 ID 그대로 사용
            title=data["title"],
...
@planner_bp.route("/stats/<user_id>", methods=["GET"])
def get_user_stats(user_id):
    """사용자 스터디 통계 조회 (문자열 ID 지원)"""
    try:
        # 이번 주 통계
        today = date.today()
